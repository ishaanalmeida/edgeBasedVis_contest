...
public class testAlgo27 extends SchematicTreesAlgorithm {
    ...
    public Output doAlgorithm(Input input) {
        ...
        for (int setIndex = 0; setIndex < input.sets.size(); setIndex++) {
            ...
            // Sort nodes by y, then x for more spatial awareness
            nodeList.sort(Comparator.comparingInt(Node::y).thenComparingInt(Node::x));
            ...
            for (int i = 0; i < nodeList.size(); i++) {
                ...
                for (int[] pos : candidates) {
                    ...
                    for (int j = Math.max(0, placed.size() - 10); j < placed.size(); j++) {
                        Position p = placed.get(j);
                        double dx = pos[0] - p.x();
                        double dy = pos[1] - p.y();
                        score += Math.sqrt(dx * dx + dy * dy);
                    }

                    // Encourage spread early, tightness late
                    score += random.nextDouble() * (1.0 / (i + 1));
