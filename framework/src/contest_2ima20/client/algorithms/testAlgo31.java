...
public class testAlgo31 extends SchematicTreesAlgorithm {
    ...
    public Output doAlgorithm(Input input) {
        ...
        for (int setIndex = 0; setIndex < input.sets.size(); setIndex++) {
            ...
            refinePositionsWithSwaps(graph, positionToNode, radius, 20);
        }
    ...
    private void refinePositionsWithSwaps(Graph graph, Map<Position, Node> positionToNode, int radius, int maxTries) {
        List<Position> positions = graph.getVertices();
        ...
        while (tries < maxTries) {
            int i = rand.nextInt(positions.size());
            int j = rand.nextInt(positions.size());
            if (i == j) continue;

            Position pi = positions.get(i);
            Position pj = positions.get(j);

            Node ni = positionToNode.get(pi);
            Node nj = positionToNode.get(pj);

            int xi = pi.x(), yi = pi.y();
            int xj = pj.x(), yj = pj.y();

            boolean validSwap = (manhattan(xj, yj, ni.x(), ni.y()) <= radius &&
                                 manhattan(xi, yi, nj.x(), nj.y()) <= radius);
            ...
            pi.set(xj, yj);
            pj.set(xi, yi);
            int newCrossings = countTotalCrossings(edges);
            pi.set(xi, yi);
            pj.set(xj, yj);
            int oldCrossings = countTotalCrossings(edges);
    ...
    private int countTotalCrossings(List<Edge> edges) {
        int crossings = 0;
        for (int i = 0; i < edges.size(); i++) {
            for (int j = i + 1; j < edges.size(); j++) {
                if (segmentsCross(edges.get(i).getStart(), edges.get(i).getEnd(),
                                  edges.get(j).getStart(), edges.get(j).getEnd())) {
                    crossings++;
                }
            }
        }
    ...
    private boolean segmentsCross(Position a, Position b, Position c, Position d) {
        return segmentsCross(a.x(), a.y(), b.x(), b.y(), c.x(), c.y(), d.x(), d.y());
    }
