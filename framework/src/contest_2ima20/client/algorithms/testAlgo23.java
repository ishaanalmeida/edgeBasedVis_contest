...
public class testAlgo23 extends SchematicTreesAlgorithm {
    ...
    public Output doAlgorithm(Input input) {
        ...
        for (int setIndex = 0; setIndex < input.sets.size(); setIndex++) {
            ...
            // Seed first node at its closest grid point
            ...
            // Place remaining nodes in batches with spiral pattern around latest MST position
            for (int i = 1; i < nodeList.size(); i++) {
                Node current = nodeList.get(i);
                Position prev = placedPositions.get(placedPositions.size() - 1);
                int bestX = -1, bestY = -1;
                ...
                for (int r = 1; r <= radius; r++) {
                    for (int dx = -r; dx <= r; dx++) {
                        ...
                        for (int sign : new int[]{-1, 1}) {
                            int x = prev.x() + dx;
                            int y = prev.y() + dy * sign;
                            if (x < 0 || y < 0 || x > width || y > height) continue;
                            String key = x + "," + y;
                            if (occupied.contains(key)) continue;

                            double score = manhattan(x, y, prev.x(), prev.y());
                            if (score < bestScore) {
                                bestScore = score;
                                bestX = x;
                                bestY = y;
